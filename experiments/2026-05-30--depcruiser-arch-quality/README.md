...
hypothesis: >
  ...
  Rust aggregator described in docs/adrs/ADR-0006-sensors.md.
---
...
- **Abstractness (A)** is not in the cruiser output by design — Martin's `D = |A + I − 1|` needs an `A` source. `docs/adrs/ADR-0006-sensors.md` already assigns that to **ts-morph**; the cruiser output alone can't compute `D`.
...
**Populate the Rust aggregator** described in `docs/adrs/ADR-0006-sensors.md` and ship it default-off (the slot is already opt-in at v0.4.0 — that doesn't need to change). Concretely:
