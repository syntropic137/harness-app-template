...
- A preserved sentrux (or other overlay) metric is proposed for promotion from "available" to a hard-enforcing gate. It may only be wired as enforcing if it passes the size-invariance admission test in [ADR-0029-fitness-metric-size-invariance](./ADR-0029-fitness-metric-size-invariance.md) § 1 (adding a well-designed module must not move it). Growth-sensitive shapes — global composite ratios, raw counts that scale with module count — stay advisory or get reshaped first. This is why `sentrux-coupling-score` (a global ratio) left the enforced MD01 set while the four size-stable sentrux metrics remained.
