...
| 11 | **`sensors-v0.3-apss.md`** (9.9 KB) | sensors v0.3 — **APSS integration** | **active 2026-05-18** | APSS `aps` binary (Rust/Python/TS/TSX) emits `.topology/metrics/{modules,functions}.json`; `harness-sensors` gates via `governance.toml`; `apss_topology` adapter shim (358 LOC); `fitness-toml-bridge` (322 LOC). v0.3 agent image **retires** grimp/depcruiser/ts-morph/sentrux. | **Covered in ADR-0006.** | Documented in ADR-0006 sensors decision. Critical architecture defined in coordination/APSS-ADR-STANDARD.md. |
...
4. **APSS integration is documented in ADR-0006-sensors.md.** The sensors v0.3 decision record details APSS as the canonical measurement layer, including topology standard contract, adapter shim architecture, and fitness-toml-bridge integration. Bead `n48.7`.
