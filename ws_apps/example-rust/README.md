...
Per `docs/standard/decisions/telemetry-sdk.md`: the Rust SDK historically defaults to gRPC/4317, but the polyglot-monorepo template pins HTTP/protobuf (4318) across all language SDKs so a single OTLP endpoint config in `OTEL_EXPORTER_OTLP_ENDPOINT` works identically for Node, Rust, and Python apps.
